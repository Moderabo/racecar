...
const std::string SERVER_ADDRESS("10.42.0.1");
const std::string CLIENT_ID("brrr_go_car");
const std::string TOPIC("commands");
...

{
	...
	// Kanseke lite kod hä'r då
	float gas_test = std::stof(msg->to_string());
	std::cout << "\ttest: " << gas_test << "\n" << std::endl;
	...
	if ( gas_test < -1 || gas_test > 1 )
	{
		return;
	}

	uint16_t x_2byte {(0xffff * (1 + gas_test)) / 2};
